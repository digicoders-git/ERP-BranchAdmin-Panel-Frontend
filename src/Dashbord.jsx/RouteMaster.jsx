...
export default function RouteMaster() {
  ...
  const fetchRoutes = async () => {
    try {
      setLoading(true);
      const { data } = await api.get('/api/route/all');
      setRoutes(data.routes || data || []);
    } catch (err) {
      showError('Error', err.response?.data?.message || 'Failed to load routes');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (editingRoute) {
        await api.put(`/api/route/update/${editingRoute._id}`, payload);
        showSuccess('Updated!', 'Route information updated successfully');
      } else {
        await api.post('/api/route/create', payload);
        showSuccess('Created!', 'New route added successfully');
      }
  ...
  const handleEdit = (route) => {
    ...
    setFormData({
      ...
      status: route.status === true || route.status === 'active' ? 'active' : 'inactive'
    });
    setShowForm(true);
  };
  ...
  const handleDelete = async (id) => {
    ...
    if (result.isConfirmed) {
      try {
        await api.delete(`/api/route/delete/${id}`);
        fetchRoutes();
  ...
  const toggleStatus = async (route) => {
    try {
      const currentStatus = route.status === true || route.status === 'active';
      const newStatus = !currentStatus;
      await api.patch(`/api/route/toggle-status/${route._id}`, { status: newStatus });
            ...
            <table className="w-full">
              ...
              <tbody>
                {loading ? (
                  ...
                  <tr key={route._id} className={`border-b hover:bg-blue-50 transition-colors ${index % 2 === 0 ? 'bg-white' : 'bg-gray-25'}`}>
                    ...
                    <td className="px-4 py-3">{route.totalDistance || 0} KM</td>
                    <td className="px-4 py-3">
                      <button
                        onClick={() => toggleStatus(route)}
                        className={`flex items-center gap-1 text-xs font-medium px-2 py-1 rounded-full ${
                          route.status === true || route.status === 'active' ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'
                        }`}
                      >
                        {route.status === true || route.status === 'active' ? <>&#x2713; Active</> : <>&#x2717; Inactive</>}
                      </button>
                    </td>
          ...
          <div className="p-8">
            {routes.length === 0 ? (
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {routes.map((route, index) => (
                  <div key={route._id} className="bg-gradient-to-br from-blue-50 to-indigo-50 rounded-2xl p-6 border-2 border-blue-200 hover:shadow-xl transition-all duration-300 hover:scale-105">
                      ...
                      <div className="flex items-center justify-between">
                        ...
                        <button
                          ...
                          className={`px-3 py-1 rounded-full text-xs font-bold ${
                            route.status === true || route.status === 'active' ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'
                          }`}
                        >
                          {route.status === true || route.status === 'active' ? 'Active' : 'Inactive'}
                        </button>
